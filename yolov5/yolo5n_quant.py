...
parser.add_argument('--weights', type=str, required=True, help='YOLO .pt weights file (e.g. yolo11n.pt)')
parser.add_argument('--model_out_dir', type=str, default='./quant_results', help='Output folder')
...
parser.add_argument('--fast_finetune', action='store_true', help='Quick finetune before calibration')
...
def quantize_yolo():
    ...
    if len(val_list) == 0:
        raise RuntimeError("Validation set (val) is missing in data.yaml")
    ...
    print("[INFO] Testing quantization...")
    total_imgs, total_dets, avg = run_inference_and_count(quant_model, val_loader, device)
    print(f"[RESULT] Images={total_imgs}, Detections={total_dets}, Avg={avg:.3f}")

    if args.quant_mode == 'calib':
        print("[INFO] Exporting quantization config...")
    ...
    try:
        ...
        print("[WARN] Could not save quant_model:", e)
